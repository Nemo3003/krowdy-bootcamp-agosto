...
class ImitandoSonyVegas{
    ...
    async joinVideos(arrVideos: any[]) {
        try {
            for (let videoSource of arrVideos) {
                console.log("🚀 ~ file: sonyvegas.controller.ts ~ line 67 ~ SonyVegasByBootcamp ~ joinVideos", videoSource)
            }
        } catch (error) {
    ...
    async addFilterVideos(arrVideos: any[]) {
        try {
            for (let videoSource of arrVideos) {
                console.log("🚀 ~ file: sonyvegas.controller.ts ~ line 67 ~ SonyVegasByBootcamp ~ addFilterVideos", videoSource)
            }
        } catch (error) {

        }
    }
    async addImagesToVideos(arrVideos: any[], getVideoSource: any){
     try {
        for (let videoSource of arrVideos) {
            console.log("🚀 ~ file: sonyvegas.controller.ts ~ line 67 ~ SonyVegasByBootcamp ~ addImagesToVideos", videoSource)
            }
            } catch (error) {
                throw error
