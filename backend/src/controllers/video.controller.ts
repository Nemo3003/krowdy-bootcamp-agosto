...
import ImitandoSonyVegas from './sonyvegas.controller'
class VideoController {
    async testingFfmpeg() {
        try {
            await ImitandoSonyVegas.ffmpeg({})
        } catch (error) {
            throw error
        }
    }
    ...
    private async setStatusOfVideo({ }: any) {
        try {
            await ImitandoSonyVegas.joinVideos({
                videoId: this.cutVideo(),
                })
                } catch (error) {
                    throw error
                    }
    ...
    async cutVideo(nameVideo: string, startTime: string, endTime: string, numberCpusAvailables: number) {
        try {
            await ImitandoSonyVegas.cutVideo(nameVideo, startTime, endTime, numberCpusAvailables)
        } catch (error) {
            throw error
        }
    }
